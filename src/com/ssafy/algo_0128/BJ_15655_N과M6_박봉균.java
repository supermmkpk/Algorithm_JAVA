 ...
 * @author 박봉균
 * @since JDK17
 * @see <a href="https://www.acmicpc.net/source/72516903"></a>
 * 
 * 결과: "맞았습니다!!", 300588KB, 868ms
 */

/*
 * 1부터 N까지 자연수(모두 다른 수) 중에서 M개를 고른 수열을 구하라.(단, 오름차)
[입력]
1) N, M (1 ≤ M ≤ N ≤ 8)
2) N개의 수. (<= 10,000)
[출력]
 ...
 */
public class BJ_15655_N과M6_박봉균 {
	static int N, M;
	/** 결과 리스트 */
	...
	static void go() {
		...
		if(list.size() == M) {
			int prev = list.get(0);
            for(int i = 1; i < M; i++) {
                if(prev > list.get(i)) { //오름차순이 아니면 종료.
                    return;
                }
                prev = list.get(i);
            }
